...
unittest {
    ...
    write("stemmer.d unittests: ");
    Stemmer stemmer;
    auto vocFile = File("data/voc.txt");
    auto expectedFile = File("data/expected.txt");
    int count = 0;
    foreach (pair; zip(vocFile.byLine, expectedFile.byLine)) {
        assert(stemmer.stem(pair[0]) == pair[1]);
        count++;
    }
    writefln("checked %,d stems", count);
}
